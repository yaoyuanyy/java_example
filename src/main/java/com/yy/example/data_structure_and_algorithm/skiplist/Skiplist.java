...
public class Skiplist {
    ...
    public void add(int num) {
        ...
        for (int i = currentLevel - 1; i >= 0; i--) {
            if (i < randomLevel) {
                ...
                oprNode = findClosestBySameLayer(oprNode, i, num);
    ...
    private boolean search(int target) {
        ...
        for (int i = currentLevel - 1; i >= 0; i--) {
            // 同一level层遍历查找小于value的最大值的skipNode
            oprNode = findClosestBySameLayer(oprNode, i, target);
            if (Objects.nonNull(oprNode.next[i]) && oprNode.next[i].value == target) {
                return true;
            }
        }
        return false;
    ...
    public boolean erase(int num) {
        ...
        for (int i = currentLevel - 1; i >= 0; i--) {
            // 同一level层遍历查找小于value的最大值的skipNode
            oprNode = findClosestBySameLayer(oprNode, i, num);
            if (Objects.nonNull(oprNode.next[i]) && oprNode.next[i].value == num) {
    ...
    public static void main(String[] args) {
        ...
        boolean result = method1.erase(10);
