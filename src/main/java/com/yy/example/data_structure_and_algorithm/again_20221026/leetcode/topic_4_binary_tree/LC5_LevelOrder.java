...
public class LC5_LevelOrder {
    ...
    public List<List<Integer>> levelOrder(TreeNode root) {
        ...
        while (!queue.isEmpty()) {
            int size = queue.size();
            System.out.println("此时 队列size:" + size);
            List<Integer> resOfLevel = new ArrayList<>();
            for (int i = 0; i < size; i++) {
                TreeNode node = queue.poll();
                resOfLevel.add(node.val);
                if(node.left != null) {
                    queue.add(node.left);
                }
                if(node.right != null) {
     ...
     * @return
     */
    public Collection<List<Integer>> levelOrder2(TreeNode root) {
        return null;
    }
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> res = new LC5_LevelOrder().levelOrder(root);
